    ...
    const fetchJobDetails = async () => {
      ...
      try {
        ...
        const processedApplications = await Promise.all(
          jobData.applications.map(async (applicationId) => {
            ...
            // Fetch artist user details using the artistProfileId
            try {
              const userResponse = await fetch(
                `${import.meta.env.VITE_API_URL}/api/users/profile/${applicationData.artistProfileId}`,
                {
                  headers: { Authorization: `Bearer ${token}` },
                }
              );
              ...
              console.warn(`Failed to fetch user details for artist ${applicationData.artistProfileId}`);
            }
            ...
            <div className="space-y-4">
              {job.applications?.length > 0 ? (
                job.applications.map((application) => (
                  <ApplicantCard key={application._id} applicant={application} />
                ))
              ) : (
